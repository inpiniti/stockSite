  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(pageBoards), (board) => {
        ...
        _push(`<div class="absolute top-0 left-0 w-full h-full lg:rounded-md" style="${ssrRenderStyle({ "background": "linear-gradient(\n                to bottom,\n                rgba(0, 0, 0, 0.5),\n                rgba(0, 0, 0, 0) 50%,\n                rgba(0, 0, 0, 0) 50%,\n                rgba(0, 0, 0, 0.5)\n              )", "pointer-events": "none" })}"></div><div class="p-4 flex w-full absolute bottom-0 text-white justify-between gap-4 items-end" style="${ssrRenderStyle({ "pointer-events": "none" })}"><div class="flex gap-4"><img class="w-fit h-24 rounded-md"${ssrRenderAttr("src", unref(KR_IMG_BOOKS)[board.kr])}><div class="flex-col"><div class="line-clamp-3 overflow-hidden">${ssrInterpolate(board.content)}</div><div class="flex text-xs gap-2"><div class="font-normal">${ssrInterpolate(("timeAgo" in _ctx ? _ctx.timeAgo : unref(timeAgo))(board.date))}</div></div></div></div><div class="flex flex-col gap-2 text-xl items-center"><div class="flex flex-col gap-1 items-center" style="${ssrRenderStyle({ "pointer-events": "auto" })}">`);
...
//# sourceMappingURL=board-JeaOinCF.mjs.map
