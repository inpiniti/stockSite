...
const paperback = usePaperback(); // 단행본
</script>
<template>
  <div class="w-full" v-if="paperback.kr">
    <div class="flex justify-center">
      <div class="grid-cols-2 grid min-w-[800px] max-w-[800px]">
        <div class="p-5"><img :src="paperback.cover_image" /></div>
        <div class="flex flex-col p-5 border-l">
          <div class="text-lg font-semibold">{{ paperback.kr }} {{ paperback.booknum }}</div>
          <div class="flex text-xl justify-end items-center gap-2 font-bold">
            <div class="text-neutral-400 line-through">{{ paperback.price || "가격미정" }} 원</div>
            <div class="text-2xl">{{ paperback.price || "가격미정" }} 원</div>
          </div>
          ...
          <div class="flex items-end justify-between">
            <div class="flex border w-fit">
              <div class="bg-neutral-200 py-2 px-4">-</div>
              <div class="py-2 px-4">1</div>
              <div class="bg-neutral-200 py-2 px-4">+</div>
            </div>
            <div>{{ paperback.price || "가격미정" }} 원</div>
          </div>
          <Separator class="my-4" />
          <div class="flex items-center justify-between pb-4">
            <div class="text-sm font-bold">총 상품 금액</div>
            <div class="flex gap-2 items-center">
              <div class="text-sm text-neutral-400">총 수량 1개</div>
              <div class="text-2xl text-red-400 font-bold">{{ paperback.price || "가격미정" }} 원</div>
            </div>
          </div>
          <div class="flex flex-col gap-2">
            <div><Button size="lg" class="w-full">구매하기</Button></div>
            <div class="flex gap-2">
              <div class="flex-1">
                <Button variant="secondary" class="w-full">찜하기</Button>
              </div>
              <div class="flex-1">
                <Button variant="outline" class="w-full">장바구니</Button>
              </div>
            </div>
          </div>
        </div>
